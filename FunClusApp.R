...
server <- function(input, output) {
  ...
  observeEvent(input$ssRun, {
    ...
    output$ssm <- DT::renderDataTable({
     ...
     #params$df_similarty <- read.csv("/home/asif/Desktop/CenturiWork/clustering/8Feb2022/Data/geneSimMatrixWithClassSelectedclass.csv")
...
#         df_sel <- input$df_for_clus
      ...
      #   print(df_fuzzy_in)
        ...
        if(!is.null(df_fuzzy_in)){
          ...
          #rownames(df_fuzzy_in) <- paste(df_fuzzy_in$class, df_fuzzy_in$genes, sep="_")
      ...
      exportOptions = list(
        ...
        searching = TRUE,
          ...
          #                     alpha = 0.9, shape = 19, geom = c("point"), ellipse = FALSE, 
          #                     main = paste("Clustering for ", input$df_for_pam,sep = "" ) ) 
#[, -which(names(df_fuzzy_in) %in% c("genes","class", "cluster"))]
          dd <- fviz_cluster(res_fanny_clus,df_fuzzy_in,
           ...
           #fuzzy_plot <- fuzzy_plot %>% style(hoverinfo = df_fuzzy_in$class, traces = 1)
           fuzzy_plot
